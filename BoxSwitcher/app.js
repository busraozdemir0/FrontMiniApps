...
function updateUser(){
    const {name, position, photo, text} = users[idx];  // birden fazla degiskene deger atama syntaxi
    ...
    if(idx > users.length - 1){  // users.length=7
        idx = 0;
    }

}

setInterval(updateUser, 10000)  // durmadan user cekme islemi yapiyoruz